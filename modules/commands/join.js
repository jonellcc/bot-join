              ...
              if (info.participantIDs.includes(handleReply.senderID)) {
                  ...
                  api.addUserToGroup(handleReply.senderID, tid, (err) => {
                      if (err) {
                          ...
                          api.sendMessage('You have been successfully added to the group check your message request or spam!', event.threadID, event.messageID);
                      }
                  });
              }
          });
...
module.exports.run = function({ api, event }) {
  ...
  api.getThreadInfo(tid, (err, info) => {
      ...
      if (info.participantIDs.includes(senderID)) {
          ...
          api.sendMessage(`Do you want to join the group chat with thread ID ${tid}? Reply with "yes" to confirm or "no" to cancel.`, event.threadID, (err, info) => {
              if (err) {
